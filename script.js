...
document.getElementById('analyze-btn').addEventListener('click', function() {
    ...
    function parseDialog(t) {
        ...
        if (hasLabels) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (s !== sender && cur) { msgs.push({ sender: sender, text: cur.trim() });
                    cur = ''; }
                sender = s;
                cur += ' ' + l;
            }
    ...
    if (!managerMsgs.length) return alert('Не найдены сообщения от "Вы"');
    ...
    rules.forEach(r => { const ok = r.ch(); total += r.w; if (ok) { passed += r.w;
    ...
    if (firstFailed) {
        err = { name: firstFailed.name, desc: neg[rules.indexOf(firstFailed)].replace('✖ ', ''), sug: firstFailed.sug };
    } else {
