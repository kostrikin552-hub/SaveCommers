...
def handle_states(user_id, chat_id, text, bot_token):
    ...
    if state.startswith('referral_withdraw'):
        if state == 'referral_withdraw_confirm':
            # Игнорируем текст в состоянии подтверждения
    ...
    try:
        if "message" in update:
            ...
            if text.startswith("/start"):
                ...
                if history:
                    ans += "Последние анализы:\n" + "\n".join([f"• {h['created_at'][:10]}: {h['score']}/100" for h in history])
                else:
                    ans += "Нет истории"
            ...
            user_id, data, chat_id = cb["from"]["id"], cb["data"], cb["message"]["chat"]["id"]
            ...
            if data == "support":
                ...
                if get_sub(user_id):
                    send_msg(chat_id, "❌ У вас уже есть подписка", bot_token=bot_token)
                else:
                    create_sub(user_id, "trial", 3)
                    send_msg(chat_id, "✅ 3 дня активированы!", bot_token=bot_token)
                answer_cb(cb["id"], bot_token=bot_token)
                ...
                if balance < 50000:
                    ...
                    user_states[user_id] = 'referral_withdraw_method'
                    withdraw_data[user_id] = {'amount': balance}
                answer_cb(cb["id"], bot_token=bot_token)
