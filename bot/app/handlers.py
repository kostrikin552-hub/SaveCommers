...
from .models_referrals import get_referral_code, get_referral_stats, apply_referral_bonus
from .utils import send_msg, answer_cb, send_error_to_admin
        ...
        if "message" in update:
            ...
            if user_id in user_states:
                ...
                if state == 'creating_company':
                    ...
                    if res:
                        send_msg(chat_id, f"🏢 Компания «{name}» создана! Код: <code>{res['invite_code']}</code>", bot_token=bot_token)
                    else:
            ...
            if text.startswith("/start"):
                ...
                if user_id == admin_id:
                    ...
                    if parts[0] == "/activate" and len(parts) >= 3:
                        ...
                        sub = get_sub(target)
                        send_msg(chat_id, f"Статус {target}: {sub['plan_type'] if sub else 'Нет'} до {sub['end_date'] if sub else '---'}", bot_token=bot_token)
