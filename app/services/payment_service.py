...
from ..repositories.payment_repo import (
    ...
    get_payment_by_idempotence, set_payment_processing, set_payment_succeeded, set_payment_failed,
    get_promo_usage_count, get_user_promo_usage
)
...
from ..config import YOOKASSA_SHOP_ID, YOOKASSA_SECRET_KEY, BASE_URL, PLANS, PROMO_CODE, PROMO_LIMIT, PROMO_PRICE
...
def create_yookassa_payment(user_id: int, plan: str, promo_code: Optional[str] = None) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    existing = execute_query(
        "SELECT payment_id, status, created_at FROM payments WHERE user_id = %s AND plan_type = %s AND status IN ('creating', 'pending', 'processing') AND created_at > NOW() - INTERVAL '10 minutes' ORDER BY created_at DESC LIMIT 1",
