...
def handle_support_message(update: Dict[str, Any]) -> None:
    ...
    user_id = message.get("from", {}).get("id")
    bot_token = update.get("bot_token")
    # Включаем режим ожидания сообщения в поддержку
    set_state(user_id, "awaiting_support_message", {})
    text = (
        "❓ <b>Помощь</b>\n\n"
        "📢 Наш канал с новостями и кейсами:\n"
        "https://t.me/SaleFlow_News\n\n"
        "✍️ Напишите ваш вопрос или предложение следующим сообщением.\n"
...
def handle_support_input(update: Dict[str, Any]) -> None:
    message = update.get("message", {})
    user_id = message.get("from", {}).get("id")
    chat_id = message.get("chat", {}).get("id")
    text = message.get("text", "").strip()
    bot_token = update.get("bot_token")
    state = get_state_data(user_id)
    # Принимать сообщение можно ТОЛЬКО после нажатия «Помощь»
    if not state or state.get("type") != "awaiting_support_message":
        return
    if not text:
        return
    try:
        ...
        user_mention = (
            f"@{username}"
            if username
            else f'<a href="tg://user?id={user_id}">{escape(first_name or "Пользователь")}</a>'
        )
        admin_text = (
            "📩 <b>Сообщение от пользователя</b>\n"
            f"ID: {user_id}\n"
            f"Имя: {user_mention}\n"
            f"Текст: {escape(text[:4000])}\n"
            f"Чат: {chat_id}"
        )
        send_msg(ADMIN_ID, admin_text, bot_token=bot_token, disable_preview=True)
        send_msg(
            chat_id,
            "✅ Ваше сообщение отправлено в поддержку. Мы ответим в ближайшее время.",
            bot_token=bot_token
        )
        # После отправки выходим из режима поддержки
        clear_state(user_id)
    except Exception:
        logger.exception("Error forwarding support message")
        send_msg(
            chat_id,
            "❌ Не удалось отправить сообщение в поддержку. Попробуйте ещё раз.",
            bot_token=bot_token
        )
