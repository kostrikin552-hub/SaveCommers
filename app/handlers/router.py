...
from .analysis import handle_analysis_message, handle_analysis_callback, handle_cases
...
from ..config import B2B_ENABLED, BOT_USERNAME
...
def process_update(update: Dict[str, Any]) -> None:
    if "callback_query" in update:
        ...
        if data.startswith("tariff_") or data == "trial":
            ...
            if B2B_ENABLED:
                # handle_company_callback(update)  # закомментировано
                pass
            else:
                from ..utils import answer_cb
            ...
            from .analysis import handle_contact_callback
        ...
        if text.startswith("/start"):
        ...
        elif text.lower() in ["🚀 новый анализ", "анализ"]:
            ...
            if B2B_ENABLED:
                ...
                send_msg(chat_id, "B2B временно недоступен", bot_token=update.get("bot_token"))
            ...
            from .start import handle_progress
        ...
        elif text.lower() == "📢 канал с кейсами":
            send_msg(chat_id, "📢 Подпишитесь на наш канал с кейсами:\nhttps://t.me/SaleFlow_Cases", bot_token=update.get("bot_token"))
        elif text.lower() == "💬 обратная связь":
            send_msg(chat_id, "💬 Напишите нам: @SaleFlow_Support\n\nМы ответим в течение 12 часов.", bot_token=update.get("bot_token"))
        elif text.lower() == "📖 примеры":
            handle_cases(update)
        else:
            from .analysis import handle_contact_input
            handle_contact_input(update)
    else:
