...
def acquire_worker_lock(lock_name: str, ttl_seconds: int = 300) -> bool:
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """INSERT INTO worker_locks (lock_name, locked_until)
...
def main_menu() -> dict:
    return {
        "keyboard": [
            ...
            [{"text": "💬 Обратная связь"}, {"text": "❓ Поддержка"}]
        ],
...
def get_state_data(user_id: int) -> Optional[dict]:
    row = execute_query(
        "SELECT data FROM user_sessions WHERE user_id = %s",
        (user_id,),
        fetch_one=True,
    )
        ...
        try:
            with get_connection() as conn:
                with transaction(conn):
                    ...
                    cur.execute(
                        """INSERT INTO companies (name, owner_id, invite_code)
                           VALUES (%s, %s, %s) RETURNING id, invite_code""",
                        (name, owner_id, code),
                    )
